...
class Category extends ValidatableModel
{
    ...
    private function handleCreatingEvent(): bool
    {
        ...
        // Use getAttribute() which works reliably in creating event
        $name = $this->getAttribute('name') ?? $this->attributes['name'] ?? null;
        if (!empty($name)) {
            $this->generateSlug();
        }
        // Calculate level based on parent or set default
        $parentId = $this->getAttribute('parent_id') ?? $this->attributes['parent_id'] ?? null;
        $level = $this->getAttribute('level') ?? $this->attributes['level'] ?? null;
    ...
    {
        // Get parent_id using getAttribute() which works reliably during events
        $parentId = $this->getAttribute('parent_id') ?? $this->attributes['parent_id'] ?? null;
        ...
        $currentLevel = $this->getAttribute('level') ?? $this->attributes['level'] ?? null;
